...
class ContentDirectoryService extends Service {
	...
	initialize(upnpServer, callback) {
		...
		super.initialize(upnpServer, (error) => {
			...
			console.log('contentDirectoryService::initialize', {contentProviders: this.contentProviders})
	...
	initializeRegistry(callback) {
		...
		console.log('contentDirectoryService::initializeRegistry', {nodeRegistry: this._nodeRegistry})
	...
	processSoap_Search(xml, request, response, callback) {
		console.log('contentDirectoryService::processSoap_search')
	...
	processSoap_Browse(xml, request, response, callback) {
		console.log('contentDirectoryService::processSoap_Browse')
	...
	processBrowseMetadata(response, request, objectId, filterSearchEngine, callback) {
		console.log('contentDirectoryService::processBrowseMetadata')
	...
	processBrowseDirectChildren(response, request, containerId, filterSearchEngine, startingIndex,
        ...
        console.log('contentDirectoryService::processBrowseDirectChildren')
	...
	browseNode(node, options, callback) {
		console.log('contentDirectoryService::browseNode', {node, options, callback})
		...
		this.asyncEmit("browse", list, node, options, (error) => {
			...
			console.log('contentDirectoryService::browseNode::asyncEmit', {list, callback})
			console.log('contentDirectoryService::browseNode::asyncEmit::list[0]', {n: list[0], a: list[0].attributes})
			callback(null, list);
		});
	}
	...
	registerNode(node, callback) {
		console.log('contentDirectoryService::registerNode', node)
	...
	sendContentURL(attributes, request, response, callback) {
		...
		contentURL.createReadStream(null, opts, (error, stream) => {
			if (error) {
				logger.error('No stream for contentURL=', contentURL, error);
...
//kept for Intel upnp toolkit, but not in upnp spec
	...
	newURL(url) {
		...
		console.log('contentDirectoryService::newURL', {url});
		if (url.charAt(0) === '/') {
